...
class AppStore extends Component {
  state = {
    stateId: tabsList[0].tabId,
    searchInput: '',
  }

  updateActiveStateId = tabId => {
    this.setState({stateId: tabId})
  }
  ...
  getFilteredApps = searchedApps => {
    ...
    const filteredApps = searchedApps.filter(
      eachApp => eachApp.category === stateId,
    )
    return filteredApps
  }
  ...
  render() {
    const {stateId, searchInput} = this.state
            ...
            <ul className="tab-container">
              {tabsList.map(eachApp => (
                <TabItem
                  key={eachApp.tabId}
                  tabDetails={eachApp}
                  updateActiveStateId={this.updateActiveStateId}
                  isActive={stateId === eachApp.tabId}
                />
              ))}
            </ul>
